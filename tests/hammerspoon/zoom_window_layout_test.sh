...
package.path = repo_root .. "/home/.config/hammerspoon/?.lua;" .. package.path
...
if rg --no-config -q 'zoomMeetingFilter|zoomNonMeetingFilter' "$config"; then
    echo "FAIL: Zoom layout still depends on cached title filters"
    exit 1
fi

if ! rg --no-config -q 'classifyZoomWindows\([^)]*:allWindows\(\)' "$config"; then
    echo "FAIL: Zoom layout does not classify a fresh allWindows snapshot"
    exit 1
fi

if rg --no-config -q 'chromeFilter' "$config"; then
    echo "FAIL: Chrome layout still depends on a cached window filter"
    exit 1
fi

if ! rg --no-config -q 'forEachUsableManagedWindow\([^)]*:allWindows\(\)' "$config"; then
