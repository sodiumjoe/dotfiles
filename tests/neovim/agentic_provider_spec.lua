describe("agentic codex provider", function()
    ...
    local function delete_tool_call_log()
        local test_state_home = vim.env.DOTFILES_TEST_STATE_HOME
        assert.is_not_nil(test_state_home)
        assert.are.equal(test_state_home .. "/nvim/agentic-codex-tool-call.log", tool_call_log)
