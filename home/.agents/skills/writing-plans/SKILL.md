...
version: 1.0.1
skill: writing-plans
content_hash: 0bc3d36590f7b2c323ed3ec18ff77e9f8ed57af42f5680a02b11a2d20de265cf
---
...
**Context:** Plans live inside a work project. Determine the project slug from `$CLAUDE_PROJECT` when present; otherwise ask which project this plan belongs to. If working in an isolated worktree, it should have been created via the `using-git-worktrees` skill at execution time.

**Save plans to:** `~/stripe/work/projects/<slug>/YYYY-MM-DD-<feature-name>.md`
...
After writing the plan, add a wikilink to the project file's `## Plans` section and open the plan in Neovim:

```bash
~/.claude/skills/neovim/scripts/nvim-open --editor ~/stripe/work/projects/<slug>/YYYY-MM-DD-<feature-name>.md
```

## Scope Check

If the work project covers multiple independent subsystems, it should have
been broken into sub-projects during brainstorming. If it was not, suggest
splitting the project before writing plans. Each plan should produce working,
...
**After its frontmatter, every plan MUST start with this header:**
...
**Spec:** [path to the work project's `project.md` that contains the
approved design — the plan argues from it, so executors read both]
...
After writing the complete plan, look at the work project with fresh eyes and check the plan against it. This is a checklist you run yourself — not a subagent dispatch.

**1. Project coverage:** Skim each section/requirement in the approved project design. Can you point to a task that implements it? List any gaps.

**2. Placeholder scan:** Search your plan for red flags — any of the patterns from the "No Placeholders" section above. Fix them.

**3. Type consistency:** Do the types, method signatures, and property names you used in later tasks match what you defined in earlier tasks? A function called `clearLayers()` in Task 3 but `clearFullLayers()` in Task 7 is a bug.

**4. Review Focus:** For each input class or failure mode the project design implies, is there a task whose tests exercise it? The five uncovered ones most likely to bite a person go in the Review Focus section, and each line there gets its test added to the owning task. An empty section means you checked and found none, not that you skipped the check.
...
**"Plan complete and saved to `~/stripe/work/projects/<slug>/<filename>.md`. Please review the plan. Which execution approach would you prefer?**
...
**"Plan complete and saved to `~/stripe/work/projects/<slug>/<filename>.md`. Please review the plan. Does it capture what you want?"**
