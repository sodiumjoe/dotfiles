...
content_hash: f38e8f2ddcf079f65493adc713c1fed78421dfaf5f4dbf6b3a6b2b1d95466e71
---
...
- Your harness has no subagent tool (see the skill-loading skill's
  per-platform references). Never fabricate a dispatch; run
  the plan here.
...
- Each plan owns a workspace: at skill start, run
  ...
  prints the plan's directory under the repository's Git metadata
  (`<git-dir>/sdd/<plan-basename>/`), home to every
...
- Removing Git metadata will destroy the workspace; if that happens,
...
When the final review is clean and its fixes are committed:
...
2. Record completion through the work CLI:
   ...
   ```
3. Delete this plan's workspace directory — the git history is the record
...
[Read plan once: ~/stripe/work/projects/<slug>/feature-plan.md; project design read]
[Resolve workspace: sdd-workspace ~/stripe/work/projects/<slug>/feature-plan.md — no ledger inside, fresh start]
