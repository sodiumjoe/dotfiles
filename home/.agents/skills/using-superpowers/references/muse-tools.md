...
Muse has native skill support via `muse skills`. To invoke an installed skill, read its `SKILL.md` and follow the instructions. The skill-loading bootstrap is injected automatically at `SessionStart` via the plugin hook — you are already following it, so do not re-load it.
