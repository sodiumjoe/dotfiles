...
describe("formatReviews", () => {
  ...
  it("formats PRs as markdown links", () => {
    ...
    const reviews = [
      {
        title: "Fix bug",
        url: "https://gh.example.com/org/repo/pull/42",
        author: "alice",
        repo: "org/repo",
        number: 42,
      },
      {
        title: "Add feature",
        url: "https://gh.example.com/org/repo/pull/99",
        author: "bob",
        repo: "org/repo",
        number: 99,
      },
    ...
    assert.equal(
      lines[0],
      "- [org/repo#42](https://gh.example.com/org/repo/pull/42) — Fix bug (alice)",
    );
    assert.equal(
      lines[1],
      "- [org/repo#99](https://gh.example.com/org/repo/pull/99) — Add feature (bob)",
    );
...
describe("injectReviews", () => {
  it("inserts Reviews section before Tasks when none exists", () => {
    ...
    assert.ok(
      content.indexOf("## Reviews") < content.indexOf("## Tasks"),
      "Reviews should be before Tasks",
    );
  ...
  it("replaces existing Reviews section", () => {
    ...
    const reviews = [
      {
        title: "New PR",
        url: "https://gh.example.com/org/repo/pull/7",
        author: "carol",
        repo: "org/repo",
        number: 7,
      },
...
describe("humanizeError", () => {
  it("detects HTML response error from GHE", () => {
    const { humanizeError } = requireFresh();
    const msg =
      "json: cannot unmarshal string into Go value: invalid character '<' looking for beginning of value";
    assert.equal(
      humanizeError(msg),
      "GHE API returned HTML instead of JSON (possible auth/connectivity issue)",
    );
  });

  it("passes through unrelated errors unchanged", () => {
    const { humanizeError } = requireFresh();
    assert.equal(humanizeError("some other error"), "some other error");
  });
